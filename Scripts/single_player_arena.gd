...
func _process(delta: float) -> void:
	...
	var milliseconds: int = int((GeneralGlobals.match_timer - total_seconds) * 1000)
	
	GeneralGlobals.time_left = str(minutes).pad_zeros(2) + ":" + str(seconds).pad_zeros(2) + "." + str(milliseconds).pad_zeros(3)
